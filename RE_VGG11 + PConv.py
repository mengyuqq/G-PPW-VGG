...
#         self.conv = CBA(32, 64, kernel_size=3, stride=2, padding=1)
#
#     def forward(self, x):
#         x0 = self.maxpool(x)
#         x1 = self.conv(x)
...
#             CBA(32, 64, kernel_size=1, stride=1),
...
class Z_2(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding=0):
        super(Z_2, self).__init__()
        self.conv = CBA(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding)
        # self.conv = CBA(64, 64, kernel_size=3, stride=2, padding=1)
