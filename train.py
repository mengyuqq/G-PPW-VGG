...
from paper1_PMConv_L3_ECANet import vgg

if os.path.exists("result/19_22m") is False:
    os.makedirs("result/19_22m")
path = "result/19_22m/"
...
train_dataset = datasets.ImageFolder(root=os.path.join(image_path, "train_m"),
                                     transform=data_transform["train"])

validate_dataset = datasets.ImageFolder(root=os.path.join(image_path, "val_m"),
...
lr_scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=10, verbose=True, min_lr=0.0, eps=1e-8)
